...
本 SDK 是 [amz-ad-pilot](#谁在用) 的数据平面底座，设计目标是可被任何需要规模化访问卖家账号的 Go 服务直接复用。
...
`amz-ad-pilot` 把这些事件落到独立的 `logs/AmzSDK/` 目录，参考实现在该项目的 `internal/datas/sdk_observer.go`。
...
> 公开发布后 `amzsdk` 即可 `go get`。在此之前以与 [`amz-ad-pilot`](#谁在用) 相同的同级目录布局消费：
...
公开发布后，下游可以删掉 `replace`，改为 `amzsdk vX.Y.Z` 直接通过公开 module path 拉取 —— 本仓库 `go.mod` 中的 module path 会同步更新。
...
> 下面示例假设你已经拿到 Amazon 颁发的 **LwA 凭据**：注册应用的 `client_id` + `client_secret`，以及通过 OAuth 授权流程换到的 per-seller `refresh_token`。
...
`amz-ad-pilot` 的 `internal/datas/sdk_observer.go` 是一个把上述事件直接落到 logrus 滚动日志的参考实现，可随意改造复用。

要打开逐请求 HTTP trace（很详细，**只**用于排障），从应用侧调用 `pkg/http_trace.go` 暴露的 helper —— 推荐参考 `amz-ad-pilot` 的 `log.sdk-http-trace` 开关。
...
- 当前固定版本为 `v1.0.0`（[amz-ad-pilot](#谁在用) 引用的版本）。
...
- 下游 `go.mod` 请固定 tag。开发期 track `master` 没问题，生产部署不推荐。

---

## 谁在用

- **amz-ad-pilot** —— AI 驱动的亚马逊 SP 广告运营平台。活动 / 广告组 / 关键词 CRUD、SP / SC 报表拉取以及大模型托管 agent 全部基于 `amzsdk`。该项目是把 `pkg.IAuth`、可观测 hook、多店 token 缓存接进真实服务的参考实现。*(`amz-ad-pilot` 公开发布后请把这一段替换成它的 GitHub 仓库链接。)*
