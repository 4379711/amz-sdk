...
func (c *App) String() string {
	return fmt.Sprintf("ClientID: %s\nClientSecret: %s\nRedirectURL: %s", c.ClientID, c.ClientSecret, c.RedirectURL)
}
...
func (a *AdAuth) GetLwaTokenEndpoint() string {
...
func (a *AdAuth) GetDataEndpoint() string {
	switch a.CountryCode {
	case "CA":
...
func (a *AdAuth) GetLwaCodeEndpoint() string {
	switch a.CountryCode {
	case "CA":
...
func (a *AdAuth) ParseToken(resp *http.Response) error {
...
// decodeTokenResponse 解析 LwA token 响应体,不修改 AdAuth 状态。
...
func (a *AdAuth) GetRefreshToken(code string) error {
...
func (h *headerInjector) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil || resp == nil {
		if err != nil {
			...
			// 用 WARN 级别帮助跨请求聚合分析。
			pkg.DefaultLogger.Warn("advertising api request network error",
				"method", req.Method,
				"url", req.URL.String(),
	...
	// 兜底重试:缓存的 ExpiresAt 与 Amazon 广告服务端判定偶尔会不一致
	// (时钟偏差/节点缓存),此时 Ad API 会返回 401/403 + "access token has expired"。
	// 这种瞬时错误由 SDK 透明处理,不应暴露给上层,否则会被业务层 IsAuthError
	// 误判为 refresh_token 吊销,错误地暂停 AI 策略。
	...
	h.setAuthHeaders(retryReq, newTk)
...
func (h *headerInjector) setAuthHeaders(req *http.Request, tk string) {
	req.Header.Set("Authorization", "Bearer "+tk)
	req.Header.Set("Amazon-Advertising-API-ClientId", h.auth.ClientID)
	req.Header.Set("Amazon-Advertising-API-Scope", h.auth.ProfileId)
}

// shouldRetryAuthExpired 仅在 Ad API 明确告知 access_token 失效时才返回 true。
// 其它 401/403 (比如 profile 无权限、签名错误) 不重试,直接抛给上层处理。
//
// 该函数会"消费"并重建 resp.Body,这样上层拿到的 resp 仍可被正常解码。
//
// Ad API 对 access_token 失效的响应 body 有多种形式:
//  1. {"message":"Unauthorized exception while handling 3P Request:
//     Unauthorized exception while authenticating LwA token: Invalid token"}
//     ——最常见,出现在 v3 campaigns/keywords/productAds 等接口
//  2. {"message":"Unauthorized"}
//     ——productMetadata 等接口会返回此简化形式
//  3. {"message":"... access token has expired ..."} / "The access token you provided has expired."
//     ——少数接口沿用 SP API 的 expired 措辞
//
// 任一条件命中即走"清缓存 + 刷新 + 重试一次"的兜底路径。
// 若重试后仍 401/403,resp 会直接返给上层,业务层的 runner 会做 backoff。
//
...
func (a *AdAuth) BuildClient() *http.Client {
